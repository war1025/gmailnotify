...
namespace GmailFeed {
   ...
   public class MailItem : GLib.Object {
       ...
       * It is intended to be embedded in a VBox along with all other messages to
       * represent the unread inbox.
       ...
       * So we need to deactivate all of the actions once an action is selected.
       * This will be reset if a connection error signal is received
       **/
      private bool actions_active;

      /**
       * The star image represents the state of the message starring. Empty means unstarred.
       * Colored means starred. A half colored star means a message is queued to toggle the state.
       * State will be toggled when a response signal is received.
      ...
      /**
       * The important image represents the state of the message importance. Empty means
       * unimportant. Colored means important. A half colored icon means a message is
       * queued to toggle the state. State will be toggled when a response signal is received.
      ...
      static construct {
         ...
         try {
            ...
            IMPORTANT_FULL  = new Gdk.Pixbuf.from_file_at_size(
                                    "%s/important_full.png".printf(base_dir), 20, 11);
            IMPORTANT_EMPTY = new Gdk.Pixbuf.from_file_at_size(
                                    "%s/important_empty.png".printf(base_dir), 20, 11);
            IMPORTANT_HALF  = new Gdk.Pixbuf.from_file_at_size(
                                    "%s/important_half.png".printf(base_dir), 20, 11);
         } catch(GLib.Error e) {
      ...
      /**
       * GMessage is the Feed representation of a message. It was not used here because it
       * incorporates no visual. It's methods also require parameters which do not make sense here.
       * This is why inheritance wasn't used.
       **/
      ...
      /**
       * Build the visual. We use EventBoxes to capture events. The three we care about
       * are enter, leave, and button press. We use bools to determine if the events should
       * alter state. This allows us to reactivate the pieces if errors happen.
       **/
      private void create_visual() {
         ...
         subject_l.wrap = true;
         subject_l.set_markup(
                        "<span foreground='#000'><b><u>%s</u></b></span>".printf(this.subject));
         ...
         read_l.set_markup(inactive_template.printf("Mark as read", " |"));
         ...
         var re = read_e.enter_notify_event.connect(() => {
            if(actions_active) {
               read_l.set_markup(hover_template.printf("Mark as read", " |"));
            }
            return false;
         });
         ...
         sigid = read_e.button_press_event.connect(() => {
            if(actions_active) {
               read_l.set_markup(active_template.printf("Marking as read...", " |"));
               mark_read_clicked();
         ...
         archive_l.set_markup(inactive_template.printf("Archive", " |"));
         ...
         var ae = archive_e.enter_notify_event.connect(() => {
            if(actions_active) {
               archive_l.set_markup(hover_template.printf("Archive", " |"));
            }
            return false;
         });
         ...
         sigid = archive_e.button_press_event.connect(() => {
            if(actions_active) {
               archive_l.set_markup(active_template.printf("Archiving...", " |"));
               archive_clicked();
               actions_active = false;
            }
            return false;
         ...
         spam_l.set_markup(inactive_template.printf("Report spam", " |"));
         ...
         var se = spam_e.enter_notify_event.connect(() => {
            if(actions_active) {
               spam_l.set_markup(hover_template.printf("Report spam", " |"));
            }
            return false;
         });
         ...
         sigid = spam_e.button_press_event.connect(() => {
            if(actions_active) {
               spam_l.set_markup(active_template.printf("Reporting spam...", " |"));
               spam_clicked();
               actions_active = false;
            }
            return false;
         ...
         trash_l.set_markup(inactive_template.printf("Delete", ""));
         ...
         var te = trash_e.enter_notify_event.connect(() => {
            if(actions_active) {
               trash_l.set_markup(hover_template.printf("Delete", ""));
            }
            return false;
         });
         ...
         sigid = trash_e.button_press_event.connect(() => {
            if(actions_active) {
               trash_l.set_markup(active_template.printf("Deleting...", ""));
               delete_clicked();
               actions_active = false;
            }
            return false;
